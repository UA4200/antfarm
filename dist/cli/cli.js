...
import { getRecentEvents, getRunEvents } from "../installer/events.js";
...
async function main() {
    ...
    if (group === "logs") {
        ...
        if (arg && /^#\d+$/.test(arg)) {
            ...
            const r = db2.prepare("SELECT id FROM runs WHERE run_number = ?").get(runNum);
            if (r) {
    ...
    if (action === "resume") {
        ...
        let run;
        if (/^\d+$/.test(target)) {
            run = db.prepare("SELECT id, run_number, workflow_id, status FROM runs WHERE run_number = ?").get(parseInt(target, 10));
        }
        if (!run) {
            run = db.prepare("SELECT id, run_number, workflow_id, status FROM runs WHERE id = ? OR id LIKE ?").get(target, `${target}%`);
        }
        if (!run) {
        ...
        if (loopStep?.loop_config) {
            ...
            if (lc.verifyEach && lc.verifyStep === failedStep.step_id) {
                ...
                db.prepare("UPDATE steps SET status = 'pending', current_story_id = NULL, retry_count = 0, updated_at = datetime('now') WHERE id = ?").run(loopStep.id);
                // Reset verify step to waiting (fires after developer completes)
                db.prepare("UPDATE steps SET status = 'waiting', current_story_id = NULL, retry_count = 0, updated_at = datetime('now') WHERE id = ?").run(failedStep.id);
        ...
        db.prepare("UPDATE steps SET status = 'pending', current_story_id = NULL, retry_count = 0, updated_at = datetime('now') WHERE id = ?").run(failedStep.id);
        // Reset run to running
        db.prepare("UPDATE runs SET status = 'running', updated_at = datetime('now') WHERE id = ?").run(run.id);
        // Ensure crons are running for this workflow
        const { loadWorkflowSpec } = await import("../installer/workflow-spec.js");
    ...
    if (action === "run") {
        ...
        process.stdout.write([`Run: #${run.runNumber} (${run.id})`, `Workflow: ${run.workflowId}`, `Task: ${run.task}`, `Status: ${run.status}`].join("\n") + "\n");
        return;
    }
