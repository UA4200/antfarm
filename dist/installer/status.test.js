...
function createTestRun(opts) {
    ...
    db.prepare("INSERT INTO runs (id, workflow_id, task, status, context, created_at, updated_at) VALUES (?, ?, ?, ?, '{}', ?, ?)").run(opts.runId, opts.workflowId, "test task", opts.status ?? "running", now, now);
    if (opts.steps) {
        for (let i = 0; i < opts.steps.length; i++) {
            const s = opts.steps[i];
            db.prepare("INSERT INTO steps (id, run_id, step_id, agent_id, step_index, input_template, expects, status, output, created_at, updated_at) VALUES (?, ?, ?, ?, ?, '', '', ?, ?, ?, ?)").run(crypto.randomUUID(), opts.runId, s.stepId, "test-agent", i, s.status, s.output ?? null, now, now);
        }
    }
}
...
describe("stopWorkflow", () => {
    ...
    it("stops a running workflow with mixed step statuses and returns correct cancelled count", async () => {
        ...
        const steps = db.prepare("SELECT step_id, status, output FROM steps WHERE run_id = ? ORDER BY step_index").all(runId);
