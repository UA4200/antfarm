 ...
 */
export function getMaxRoleTimeoutSeconds() {
...
function buildToolsConfig(role) {
    ...
    const tools = {};
    if (defaults.profile)
        tools.profile = defaults.profile;
    if (defaults.alsoAllow?.length)
        tools.alsoAllow = defaults.alsoAllow;
    tools.deny = defaults.deny;
    return tools;
}
function ensureCronSessionRetention(config) {
    if (!config.cron)
        config.cron = {};
...
function ensureSessionMaintenance(config) {
    if (!config.session)
        config.session = {};
