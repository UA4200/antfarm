 ...
 */
export function selectAntfarmManagedAgents(list, workflowIds, workflowWorkspaceRoot = resolveWorkflowWorkspaceRoot()) {
    const knownWorkflowIds = new Set(Array.from(workflowIds)
        .map((id) => id.trim())
        .filter(Boolean));
    return list.filter((entry) => {
        const id = typeof entry.id === "string" ? entry.id : "";
...
async function listInstalledWorkflowIds() {
...
export async function uninstallAllWorkflows() {
    ...
    if (config.session?.maintenance) {
        ...
        const matchesDefaults = maintenance.mode === DEFAULT_SESSION_MAINTENANCE.mode &&
    ...
    if (await pathExists(antfarmRoot)) {
        ...
        const entries = await fs.readdir(antfarmRoot).catch(() => ["placeholder"]);
        if (entries.length === 0) {
