 ...
 */
export function checkStuckSteps() {
    const db = getDb();
    const findings = [];
  ...
  `).all(MAX_ROLE_TIMEOUT_MS);
    for (const step of stuck) {
        const ageMin = Math.round((Date.now() - new Date(step.updated_at).getTime()) / 60000);
        findings.push({
            check: "stuck_steps",
            severity: "warning",
 ...
 */
export function checkStalledRuns() {
    const db = getDb();
    const findings = [];
  ...
  `).all(STALL_THRESHOLD_MS);
    for (const run of stalled) {
        const ageMin = Math.round((Date.now() - new Date(run.last_step_update).getTime()) / 60000);
        findings.push({
            check: "stalled_runs",
            severity: "critical",
...
export function checkDeadRuns() {
  ...
  `).all();
    for (const run of zombies) {
        ...
        const failed = db.prepare("SELECT COUNT(*) as cnt FROM steps WHERE run_id = ? AND status = 'failed'").get(run.id);
        const action = "fail_run";
 ...
 */
export function checkOrphanedCrons(cronJobs) {
    const db = getDb();
    const findings = [];
    ...
    for (const wfId of workflowIds) {
        const active = db.prepare("SELECT COUNT(*) as cnt FROM runs WHERE workflow_id = ? AND status = 'running'").get(wfId);
        if (active.cnt === 0) {
