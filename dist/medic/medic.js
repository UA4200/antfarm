...
import { runSyncChecks, checkOrphanedCrons, } from "./checks.js";
import crypto from "node:crypto";
// ── DB Migration ────────────────────────────────────────────────────
export function ensureMedicTables() {
    const db = getDb();
    db.exec(`
...
async function remediate(finding) {
    ...
    switch (finding.action) {
        case "reset_step": {
            ...
            const step = db.prepare("SELECT abandoned_count FROM steps WHERE id = ?").get(finding.stepId);
            if (!step)
                return false;
            ...
            if (newCount >= 5) {
                db.prepare("UPDATE steps SET status = 'failed', output = 'Medic: abandoned too many times', abandoned_count = ?, updated_at = datetime('now') WHERE id = ?").run(newCount, finding.stepId);
                if (finding.runId) {
                    db.prepare("UPDATE runs SET status = 'failed', updated_at = datetime('now') WHERE id = ?").run(finding.runId);
                    emitEvent({
            ...
            db.prepare("UPDATE steps SET status = 'pending', abandoned_count = ?, updated_at = datetime('now') WHERE id = ?").run(newCount, finding.stepId);
            if (finding.runId) {
                emitEvent({
                    ts: new Date().toISOString(),
                    event: "step.timeout",
        ...
        case "fail_run": {
            if (!finding.runId)
                return false;
            const run = db.prepare("SELECT status, workflow_id FROM runs WHERE id = ?").get(finding.runId);
            if (!run || run.status !== "running")
                return false;
            db.prepare("UPDATE runs SET status = 'failed', updated_at = datetime('now') WHERE id = ?").run(finding.runId);
            // Also fail any non-terminal steps
            db.prepare("UPDATE steps SET status = 'failed', output = 'Medic: run marked as dead', updated_at = datetime('now') WHERE run_id = ? AND status IN ('waiting', 'pending', 'running')").run(finding.runId);
            emitEvent({
                ts: new Date().toISOString(),
                event: "run.failed",
                runId: finding.runId,
 ...
 */
export async function runMedicCheck() {
    ensureMedicTables();
    // Gather all findings
    ...
    if (findings.length === 0) {
        ...
        if (actionsTaken > 0)
            parts.push(`${actionsTaken} auto-fixed`);
    }
...
export function getMedicStatus() {
    try {
        ensureMedicTables();
        const db = getDb();
        const last = db.prepare("SELECT checked_at, summary, issues_found, actions_taken FROM medic_checks ORDER BY checked_at DESC LIMIT 1").get();
...
export function getRecentMedicChecks(limit = 20) {
    try {
        ensureMedicTables();
        const db = getDb();
        const rows = db.prepare("SELECT * FROM medic_checks ORDER BY checked_at DESC LIMIT ?").all(limit);
        return rows.map(r => ({
            id: r.id,
